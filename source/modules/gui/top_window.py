...
class ApplicationGUI(ttk.Frame):
    ...
    def _construct_right_frame(self):
        ...
        self.stop_button = ttk.Button(
            ...
            command=self.click_stop_monitoring,
            state=tk.DISABLED,
        )
    ...
    def _construct_left_frame(self):
        ...
        # AWS IAM Settings
        frame_session = ttk.LabelFrame(frame_left, text="AWS IAM Settings", width=280)
        self.session_list = ["default"]
        self.session_combobox = ttk.Combobox(
            frame_session, values=self.session_list, state="readonly", exportselection=0
        )
        self.session_combobox.pack(padx=5, pady=5, anchor=tk.W, side=tk.LEFT)
        self.session_combobox.set(self.session_list[0])
        session_edit_button = ttk.Button(
            frame_session, text="Edit", command=self.edit_credentials, state=tk.DISABLED
        )
        session_edit_button.pack(padx=5, pady=5, anchor=tk.W, side=tk.LEFT)
        session_add_button = ttk.Button(
            frame_session, text="Add", command=self.add_credentials, state=tk.DISABLED
        )
        session_add_button.pack(padx=5, pady=5, anchor=tk.W, side=tk.LEFT)
